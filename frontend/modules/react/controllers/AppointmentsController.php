...
{	
	public function actionVideocall()    {
        ...
        if (Yii::$app->request->post()) 
        {
            ...
            else
            {
				...
				if(empty($model))
				{
					$mobile = User::find()->where(['access_token'=>$post['access_token']])->one();					
					...
					$message = $resultDynamic->data->meetingUrl.'/'.$mobile->username;
	...
	public function actionVisitids()    {
		$data = array();
		header('Access-Control-Allow-Origin: *');
		header('Access-Control-Allow-Methods: *');
		header('Access-Control-Allow-Headers: *');
        ...
        if (Yii::$app->request->post()) 
        {
            ...
            {
                $access_token = str_replace('Bearer ','',$Authorization);
				$model = Orders::find()->select('visitId')->where(['access_token'=>$post['access_token']])->andWhere(['!=','visitId','NULL'])->orderBy('createdDate DESC')->all();
				return ['status' => true, 'message' => 'Success','data'=>$model]; 
	         }
        }
    }
	...
	public static function actionGlucosechart()	{
	   ...
	   else
	   {
			...
			if($get['type'] == 'week')
			{
				...
				$start_week = date("Y-m-d",$start_week);
				$end_week = date("Y-m-d");
				$dates = date('d-m-Y',strtotime($start_week)).' To '.date('d-m-Y',strtotime($end_week));
				$i= 0;
			...
			if($get['type'] == 'month')
			{
				...
				$currentMonth = date('m');	
				if($yourMonth == 12	&& 	$currentMonth == 1)
				{	
					$dateyear = date('Y') -1;
					$firstweekstart = $dateyear.'-'.$yourMonth.'-'.date('d');
				}
				else
				{
				...
				$dates = date('d-m-Y',strtotime($firstweekstart)).' To '.date('d-m-Y');
				...
				for($i=0;$i<4;$i++)
				{
					...
					$name = date('d-m-Y',strtotime($start_week))." - ".date('d-m-Y',strtotime($end_week));
			...
			if($get['type'] == 'year')
			{
				...
				$dates = date('d-m-Y',strtotime($date)).' To '.date('d-m-Y');				
				$currentYear = date('Y');
	...
	public function view($access_token)	{
		...
		$date = date('Y-m-01',strtotime(date('Y-m-d')));
		$data['deviceuserd'] = "";
		$order = Orders::find()->where(['access_token'=>$access_token])->andWhere(['!=','visitId','NULL'])->count();
		if($order == 0)
		{
			$data['visitId'] = "No";
		}
	...
	public static function actionWeightchart()	{
	   ...
	   else
	   {
			...
			$dates = date('d-m-Y');
			$bmiarray = [];
			$heightnew = Bmivalues::find()->where(['access_token'=>$access_token])->one();
			if(!empty($heightnew))
			{
				$height = $heightnew->height;
			}
			else
			{
				$height  = 0;
	...
	public static function actionHbaonecchart()	{
		$data = array();
		$get = Yii::$app->request->get();
		header('Access-Control-Allow-Origin: *');
		header('Access-Control-Allow-Methods: *');
	   ...
	   else
	   {
			...
			if($get['type'] == 'day')
			{
				$hba1cavg = Bmivalues::find()->where(['createdDate'=>date('Y-m-d'),'access_token'=>$access_token])->average('hba1c');
				$data[0]['name'] = date('Y-m-d');
				$data[0]['HBA1c'] = number_format($hba1cavg,1);
			...
			if($get['type'] == 'week')
			{
				...
				{
						$data[$i]['name'] = date("l",strtotime($start_week));
						$hba1cavg = Bmivalues::find()->where(['createdDate'=>$start_week,'access_token'=>$access_token])->average('hba1c');
						$data[$i]['HBA1c'] = number_format($hba1cavg,1);
						$i++;
						$start_week = date('Y-m-d',strtotime("+1 day", strtotime($start_week)));
				}			 
			}
			if($get['type'] == 'month')
			{
				...
				for($i=0;$i<4;$i++)
				{
					...
					$data[$i]['name'] = $name;
					$hba1cavg= Bmivalues::find()->where(['between', 'createdDate', $start_week, $end_week ])->andwhere(['access_token'=>$access_token])->average('hba1c');
					$data[$i]['HBA1c'] = number_format($hba1cavg,1);					
				}
			}
			if($get['type'] == 'year')
			{
				...
				{						
						...
						$currentYear = date("Y",strtotime($date));
						$currentMonth = date("m",strtotime($date));
						$hba1cavg = Bmivalues::find()->where(" Year( createdDate) = $currentYear ")->andwhere(" Month( createdDate) = $currentMonth ")->andwhere(['access_token'=>$access_token])->average('hba1c');
						$data[$i]['HBA1c'] = number_format($hba1cavg,1);
						$date = date('Y-m-d',strtotime("+1 month", strtotime($date)));
				}
			}
        ...
        else
        {
				...
				if($userplans != [])
				{
						...
						{	
							$newdata = [];
							$orderitemarray = [];
							$orderbookings = Orders::find()->where(['access_token'=>$value->access_token])->andWhere(['>=','slotDate',$value->createdDate])->andWhere(['!=','bookingStatus','Cancel'])->orderBy('orderId DESC')->all();
							if(!empty($orderbookings))
							{
								foreach($orderbookings as $orderkey=>$ordervalue)
									{
										...
										$missed = Orders::find()->leftjoin('orderitems','orderitems.orderId=orders.orderId')->andwhere(['IN','itemId',$upcomingdetailstexts])->where(['orders.access_token'=>$value->access_token,'orders.orderId'=>$ordervalue->orderId])->andwhere(['<','slotDate',date('Y-m-d')])->count();
										if($missed > 0)
										{
											$missedcount = $missedcount +1;
										}
	...
	public function diagnosticscalinformation($value,$orderitemarray)	
	{
		...
		$franchies = User::find()->where(['id'=>$value->clinicId])->one();
		if(!empty($franchies))
		{
			$data['doctorName'] = $franchies->username;
		}
		else
		{
			$data['doctorName'] = "";
	...
	public function actionTestbooking()
	{
        ...
        else
        {
				...
				$status = 'Booked';
				$notification = $this->notificationfortestbooking($access_token,$post['slotDate'],$status,$user->username);					
				return ['status' => true, 'message' => 'Success'];          
		}
	}	
	...
	public function actionTestreshedule()    {
        ...
        if (Yii::$app->request->post()) 
        {
            ...
            else
            {
				...
				$model->save();	
				$status = 'Reshedule';
				$notification = $this->notificationfortestbooking($model->access_token,$post['slotDate'],$status,$user->username);					
						...
						foreach($userplans as $key=>$value)
						{
							...
							if(!empty($orderbookings))
							{
								...
								foreach($orderbookings as $orderkey=>$ordervalue)
								{
										...
										$doctor = User::find()->where(['id'=>$value->clinicId])->one();
										$testnewdata['mobilenumber'] = $usernew->username;
										$testnewdata['doctorName'] = $doctor->username;
	...
	public function doctorcalinformation($value)	
	{
		...
		$data['name'] = $profile['firstName'];
		$data['patId'] = $profile['userId'];
		$order = Orders::find()->where(['access_token'=>$value->access_token])->andWhere(['!=','visitId','NULL'])->count();
		if($order == 0)
		{
			$data['visitId'] = "No";
		}
		...
		if($users != [])
		{
			...
			{	
				...
				foreach($upcomingdetailsarraydata as $k=>$v)
				{
						...
						$start = date('Y-m-d',strtotime("+".$v['day']." day", strtotime($value->updatedDate)));
						$end = date('Y-m-d',strtotime("+".$v['endday']." day", strtotime($value->updatedDate)));
						$model = Slotbooking::find()->where(['access_token'=>$value->access_token])->andwhere(['BETWEEN','slotDate',$start,$end])->andwhere(['!=','status','Cancel'])->orderBy('bookingId DESC')->all();
						if($model != [])
						{
							$oldend = $end;
							foreach($model as $km=>$mv)
							{
								$usernew = User::find()->where(['access_token'=>$value->access_token])->one();
								$profile = Userprofile::find()->where(['userId'=>$usernew->id])->one();
								$data['tile'] = $profile->firstName; 
								$data['access_token'] = $profile->access_token; 
								$data['start'] = $mv->slotDate.'T'.date('H:i:s',strtotime($mv->slotTime));
								$data['end'] = $mv->slotDate.'T'.date('H:i:s',strtotime($mv->slotTime));
								$data['status'] = $mv->status;
								$data['slotId'] = $mv->slotId;
								$data['bookingId'] = $mv->bookingId;
								$data['age'] = $profile['age'];
								$data['mobilenumber'] = $usernew->username;
								$data['gender'] = $profile['gender'];
								$data['name'] = $profile['firstName'];
								...
								if(!empty($date))
								{
									$data['lastreview'] = date('M d,Y',strtotime($date->slotDate));
								}
								else
								{
									$data['lastreview'] = date('M d,Y',strtotime(date('Y-m-d')));
								}
								if(($mv->status == 'Booked' || $mv->status == 'Reshedule') && $mv->slotDate >= date('Y-m-d'))
								{
									$upcomingcount = $upcomingcount +1;
								}
								elseif(($mv->status == 'Booked' || $mv->status == 'Reshedule') && $mv->slotDate < date('Y-m-d'))
								{
									$missedcount = $missedcount +1;
									$data['status'] = "Missed";
								}
						...
						{				
								$newmodel = Slotbooking::find()->where(['access_token'=>$value->access_token])->andwhere(['>','slotDate',$oldend])->andwhere(['!=','status','Cancel'])->orderBy('bookingId DESC')->all();
								if(!empty($newmodel))
								{
									$oldend = $end;
									foreach($newmodel as $km=>$mvm)
									{
										$usernew = User::find()->where(['access_token'=>$value->access_token])->one();
										$profile = Userprofile::find()->where(['userId'=>$usernew->id])->one();
										$data['tile'] = $profile->firstName; 
										$data['access_token'] = $profile->access_token; 
										$data['start'] = $mvm->slotDate.'T'.date('H:i:s',strtotime($mvm->slotTime));
										$data['end'] = $mvm->slotDate.'T'.date('H:i:s',strtotime($mvm->slotTime));
										$data['status'] = $mvm->status;
										$data['slotId'] = $mvm->slotId;
										$data['bookingId'] = $mvm->bookingId;
										$data['age'] = $profile['age'];
										$data['mobilenumber'] = $usernew->username;
										$data['gender'] = $profile['gender'];
										$data['name'] = $profile['firstName'];
										...
										if(($mvm->status == 'Booked' || $mvm->status == 'Reshedule') && $mvm->slotDate >= date('Y-m-d'))
										{
											$upcomingcount = $upcomingcount +1;
										}
										elseif(($mvm->status == 'Booked' || $mvm->status == 'Reshedule') && $mvm->slotDate < date('Y-m-d'))
										{											
											$missedcount = $missedcount +1;
											$data['status'] = "Missed";
										}
		...
		if($users != [])
		{
			...
			{		
				...
				foreach($upcomingdetailsarraydata as $k=>$v)
				{
						...
						$start = date('Y-m-d',strtotime("+".$v['day']." day", strtotime($value->updatedDate)));
						$end = date('Y-m-d',strtotime("+".$v['endday']." day", strtotime($value->updatedDate)));
						$model = Dieticianslotbooking::find()->where(['access_token'=>$value->access_token])->andwhere(['BETWEEN','slotDate',$start,$end])->andwhere(['!=','status','Cancel'])->orderBy('bookingId DESC')->all();
						if($model != [])
						{
							$oldend = $end;
							foreach($model as $km=>$mv)
							{
								...
								$date = Dieticianslotbooking::find()->where(['dieticianId'=>$doctor->dieticianId,'access_token'=>$value->access_token])->orderBy('bookingId DESC')->one();
								$data['deviceuserd'] = "";
								if(!empty($date))
								{
									$data['lastreview'] = date('M d,Y',strtotime($date->slotDate));
						...
						{				
								$newmodel = Dieticianslotbooking::find()->where(['access_token'=>$value->access_token])->andwhere(['NOT BETWEEN','slotDate',$start,$end])->andwhere(['>','slotDate',$oldend])->andwhere(['!=','status','Cancel'])->orderBy('bookingId DESC')->all();
								if(!empty($newmodel))
								{
									$oldend = $end;
									foreach($newmodel as $km=>$mvm)
        ...
        if (Yii::$app->request->post()) 
        {
            ...
            else
            {
				...
				if(empty($newmodel))
				{
					...
					$status = 'Reshedule';
					$notification = $this->notificationforappointment($model->access_token,$doctor->doctorName,$model->slotDate,$model->slotTime,$status,$user->username);					
				}
				else
				{
            ...
            else
            {
              //  $access_token = str_replace('Bearer ','',$Authorization);
				...
				if(empty($newmodel))
				{
					...
					$status = 'Reshedule';
					$notification = $this->notificationforappointment($model->access_token,$doctor->dieticianName,$post['slotDate'],$post['slotTime'],$status,$user->username);					
	...
	public function actionDieticianSlotcancel()    {        
        ...
        if (Yii::$app->request->post()) 
        {
            ...
            else
            {
				...
				$doctor = Dietician::find()->where(['dieticianId'=>$model->dieticianId])->one();
				$status = 'Cancel';
				$notification = $this->notificationforappointment($model->access_token,$doctor->dieticianName,$model->slotDate,$model->slotTime,$status,$user->username);					
        ...
        if (Yii::$app->request->post()) 
        {
            ...
            else
            {
				...
				if(empty($model))
				{
					...
					$model->save();
					$status = 'Booked';
					$notification = $this->notificationforappointment($access_token,$doctor->doctorName,$post['slotDate'],$post['slotTime'],$status,$user->username);					
				}
				else
				{
						return ['status' => false, 'message' => 'Already Slot Is booked Please check another slot']; 
	...
	public function notificationforappointment($access_token,$doctor,$date,$time,$status,$name)
	{
		...
		if($status == 'Booked')
		{
			$data['body'] = "Your appointment has been booked with ".$doctor." on ".$date." at ".$time.", By ".$name;
		}
		elseif($status == 'Reshedule')
		{
			$data['body'] = "Your appointment has been rescheduled to ".$date." at ".$time." By ".$name;
		}
		elseif($status == 'Cancel')
		{
			$data['body'] = "Your appointment with ".$doctor." has been Cancel on ".$date." at ".$time.", By ".$name;
		}
	...
	public function notificationfortestbooking($access_token,$date,$status,$name)
	{
		//print_r($access_token);exit;
		date_default_timezone_set("Asia/Calcutta");
		$data = [];
		...
		if($status == 'Booked')
		{
			$data['body'] = "Your appointment has been booked for lab tests at ".$name." on ".$date." & ".$time;
		}
		elseif($status == 'Reshedule')
		{
			$data['body'] = "Your appointment for lab tests has been rescheduled to ".$date." & ".$time." at ".$name;
		}
		elseif($status == 'Cancel')
		{
			$data['body'] = "Your appointment with ".$doctor." has been Cancel on ".$date." at ".$time.", By ".$name;
        ...
        if (Yii::$app->request->post()) 
        {
            ...
            else
            {
				...
				if(empty($model))
				{
					...
					$model->save();
					$status = 'Booked';
					$notification = $this->notificationforappointment($access_token,$doctor->dieticianName,$post['slotDate'],$post['slotTime'],$status,$user->username);					
	...
	public function actionLoginstatus()
	{
		...
		{
			$login = Login::find()->where(['userId'=>$get['patId']])->count();
			if($login > 0)
			{
				return ['status' => true, 'message' => 'Patient Already Login'];
			}
			else
			{
				return ['status' => false, 'message' => 'Patient Not Yet Login'];
			}
		}
	}
}
