...
class SiteController extends Controller
{
	...
	public function actionAdduser()    {       
        ...
        if(empty($profile))
		{
				...
				//print_r($newprofile->save());exit;
				if(!$newprofile->save()){
					print_r($newprofile->errors);exit;
				}
		}
	...
	public static function actionPercentagevalues()    {
		header('Access-Control-Allow-Origin: *');
		header('Access-Control-Allow-Methods: *');
		header('Access-Control-Allow-Headers: *');
        $data = array();
        ...
        if (empty($Authorization))
        {
            ...
            else
            {
				...
				return ['status' => true, 'message' => 'success','forceupdate'=>$require,'planstatus'=>$planstatus,'webinarcount'=>$webinarcount,'name'=>$profile['firstName'],'enrollmentstatus'=>$enrollmentstatus,'profilestatus'=>$profilestatus,
	...
	public function actionSubcribeplannew()    { 
	    ...
	    if (Yii::$app->request->post()) 
        {
            ...
            else
            {
                ...
                return ['status' => true, 'message' => 'Subcribe success Please click on link https://uatapp.apollohl.in/sugar/'];              
            }
        }
    }
	public function actionSubcribeplanBKP()    {   
        $data = array();
		header('Access-Control-Allow-Origin: *');
		header('Access-Control-Allow-Methods: *');
		header('Access-Control-Allow-Headers: *');
	    ...
	    if (Yii::$app->request->post()) 
        {
            ...
            else
            {
				...
				else
				{
					...
					{
						$users = User::find()->leftjoin('doctors','doctors.userId=user.id')->where(['roleId'=>3,'doctors.clinicId'=>$clinicId])->all();
					}					
				...
				if(empty($clinicId))
				{
					$dieticians = User::find()->where(['roleId'=>5])->all();
				}
				else
				{
					$dieticians = User::find()->leftjoin('dietician','dietician.userId=user.id')->where(['roleId'=>5,'dietician.clinicId'=>$clinicId])->all();
				}				
				...
				$plan = Userplans::find()->where(['planId'=>$post['planId'],'access_token'=>$access_token])->one();
				if(empty($plan))
				{
					$model = new Userplans();
					$price = Plans::find()->where(['planId'=>$post['planId']])->one();
					...
					{						
					  ...
					  {						 
								foreach($details as $x=>$v)
								{
									...
									$statusnew['textname'] = 'Doctor Consultation';
	...
	public function actionUpcomingeventsnew()	{
		header('Access-Control-Allow-Origin: *');
		header('Access-Control-Allow-Methods: *');
		header('Access-Control-Allow-Headers: *');
		$data = array();
        ...
        if (empty($Authorization))
        {
           ...
           $user = User::find()
            ...
            else
            {
				...
				$upcomingdetails = Plandetails::find()->where(['<=','day',$days_between])->where(['>=','endday',$days_between])->andwhere(['planId'=>$newmodel->planId])->asArray()->one();
				if(!empty($upcomingdetails))
				{
					if($newmodel->updatedDate == date('Y-m-d'))
					{
						$upcomingdetails['day'] = date('M d,Y',strtotime($newmodel->updatedDate));				
					}
					else
					{
				...
				$completebooking = Slotbooking::find()->where(['access_token'=>str_replace('Bearer ','',$Authorization)])->andWhere(['Status'=>"Completed"])->andwhere(['between','slotDate',date('Y-m-d',strtotime($upcomingdetails['day'])),date('Y-m-d',strtotime($upcomingdetails['endday']))])->orderBy('bookingId DESC')->one();
				$dieticianbooking = Dieticianslotbooking::find()->where(['access_token'=>str_replace('Bearer ','',$Authorization)])->andWhere(['Status'=>"Completed"])->andwhere(['between','slotDate',date('Y-m-d',strtotime($upcomingdetails['day'])),date('Y-m-d',strtotime($upcomingdetails['endday']))])->orderBy('bookingId DESC')->one();
				...
				if((empty($completebooking)))
				{
					...
					$booking = Slotbooking::find()->where(['access_token'=>str_replace('Bearer ','',$Authorization)])->andWhere(['!=','Status',"Cancel"])->andwhere(['between','slotDate',date('Y-m-d',strtotime($upcomingdetails['day'])),date('Y-m-d',strtotime($upcomingdetails['endday']))])->orderBy('bookingId DESC')->one();
				...
				elseif(empty($dieticianbooking) && $newmodel->dieticianId != 0)
				{
					...
					$upcomingdetails['exp'] = $doctorprofile->experience;
					$upcomingdetails['qualification'] = $doctorprofile->qualification;	
					$booking = Dieticianslotbooking::find()->where(['access_token'=>str_replace('Bearer ','',$Authorization)])->andWhere(['!=','Status',"Cancel"])->andwhere(['between','slotDate',date('Y-m-d',strtotime($upcomingdetails['day'])),date('Y-m-d',strtotime($upcomingdetails['endday']))])->orderBy('bookingId DESC')->one();
				...
				else
				{
					...
					$orderbooking = Orders::find()->where(['access_token'=>str_replace('Bearer ','',$Authorization)])->andwhere(['between','slotDate',date('Y-m-d',strtotime($upcomingdetails['day'])),date('Y-m-d',strtotime($upcomingdetails['endday']))])->andwhere(['!=','bookingStatus','Report Generated'])->orderBy('orderId DESC')->one();
					...
					$textarray[1] = 2;
					$orderitemarray = [];
					$orderbookings = Orders::find()->where(['access_token'=>str_replace('Bearer ','',$Authorization)])->andwhere(['between','slotDate',date('Y-m-d',strtotime($upcomingdetails['day'])),date('Y-m-d',strtotime($upcomingdetails['endday']))])->andwhere(['bookingStatus'=>'Reports Generated'])->orderBy('orderId DESC')->all();
					//$orderbookings = Orders::find()->where(['access_token'=>str_replace('Bearer ','',$Authorization)])->andwhere(['between','slotDate',date('Y-m-d',strtotime($upcomingdetails['day'])),date('Y-m-d',strtotime($upcomingdetails['endday']))])->orderBy('orderId DESC')->all();
...
//$orderitem = ItemDetails::find()->where(['itemNewid'=>$order->itemId])->one();
					...
					$newtest = Plandetails::find()->where(['NOT IN','text',$textarray])->andwhere(['planId'=>$newmodel->planId])->one();
					$upcomingdetailsarray = [];
					if($newtest!= [])
					{
					...
					{
						date_default_timezone_set("Asia/Calcutta");
						$query = Orders::find()->leftjoin('orderitems','orderitems.orderId=orders.orderId')->where(['slotDate'=>date('Y-m-d'),'itemId'=>$upcomingdetailsarray[0]['testId'],'orders.access_token'=>$user->access_token])->one();
				        if(empty($query))
						{
				...
				else
				{
					...
					$upcomingdetails['doctorName'] = '';					
					$upcomingdetails['doctorId'] = '';
					$upcomingdetails['exp'] = '';
					$upcomingdetails['qualification'] = '';
				}
				return ['status' => true,'message' => 'Plans','PurchaseDate'=>$newmodel->createdDate,'plan'=>$plan,'doctorname'=>$doctorname,'dieticanName'=>$dieticanName,'upcomingdetails'=>$upcomingdetails,'diagnosticbooking'=>false,'Tests'=>$upcomingdetailsarray]; 
			}
			else
			{
	...
	public function actionUpcomingeventsbkp()	{
		header('Access-Control-Allow-Origin: *');
		header('Access-Control-Allow-Methods: *');
		header('Access-Control-Allow-Headers: *');
		$data = array();
        ...
        if (empty($Authorization))
        {
           ...
           $user = User::find()
            ...
            else
            {
				...
				$testarray = [];
				$upcomingdetailsarray = [];
				$upcomingdetails = Plandetails::find()->where(['>=','day',$days_between])->andwhere(['planId'=>$newmodel->planId])->asArray()->one();
				if($upcomingdetails != [])
				{
					$events = Plandetails::find()->where(['planId'=>$newmodel->planId,'day'=>$upcomingdetails['day'],'endday'=>$upcomingdetails['endday']])->all();
					if($newmodel->updatedDate == date('Y-m-d'))
					{
						$upcomingdetails['day'] = date('M d,Y',strtotime($newmodel->updatedDate));
					}
					...
					if(!empty($events))
					{
							foreach($events as $key=>$value)
							{
								if($value->text == 1)
								{
									$doctorconsultation = Slotbooking::find()->where(['access_token'=>str_replace('Bearer ','',$Authorization)])->andwhere(['between','slotDate',date('Y-m-d',strtotime($upcomingdetails['day'])),date('Y-m-d',strtotime($upcomingdetails['endday']))])->orderBy('bookingId DESC')->one();
