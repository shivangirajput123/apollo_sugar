...
$this->params['breadcrumbs'][] = ['label' => 'Dietician', 'url' => ['index']];
...
<?php $url = Yii::$app->urlManager->createUrl(['users/dietician/getslots']) ?>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js"></script>
<script>
$(document).ready(function () {
  $('#dslots-slotdate').on('change',function(){
	  var date =  $('#dslots-slotdate').val();
	  window.location.href='<?= Url::to(['dietician/slots','id'=>$_GET['id']])?>&date='+date+'&enddate='+date;
	 
  });
  
  
  $('#dslots-duration').on('change',function(){
		var duration = $(this).val();
		var timings = $('#dslots-timings').val();
		if(timings != '' &&  duration != ''){
			$.ajax({
                 type: "GET",
                 url: '<?php echo $url;?>',
	...
	$('#dslots-timings').on('change',function(){
		var timings = $(this).val();
		var duration = $('#dslots-duration').val();
		if(timings != '' &&  duration != ''){
			$.ajax({
                 type: "GET",
                 url: '<?php echo $url;?>',
